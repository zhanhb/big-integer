...
void test0() {
    ...
    for (;; ++ch) {
        BigInteger x(ch);
        BigInteger a = BigInteger(0) + x + BigInteger(0);
        assert(a == x);
        assert(BigInteger(0) + x + BigInteger(0) == x);
        if (x == SCHAR_MAX)break;
    }
    int ii = INT_MIN;
    for (int i = 0; i < 10000; ++ii, ++i) {
        BigInteger x = ii;
        BigInteger a = BigInteger(0) + x + 0;
        assert(a == x);
        assert(BigInteger(0) + x + 0 == x);
    }
}
...
void testCalc2() {
    for (int cas = 500000; cas--;) {
        LL a = (myrandom.nextInt() & 0x1FFFF) - 0xFFFF;
        LL b = (myrandom.nextInt() & 0x1FFFF) - 0xFFFF;

        BigInteger aa(a), bb(b);
        assert(BigInteger(a + b) == aa + bb);
        assert(BigInteger(a - b) == aa - bb);
        assert(BigInteger(a * b) == aa * bb);
...
void testDivide() {
    for (int i = 0; i < 100; ++i) {
        ...
        for (int j = 0; j < 1000; ++j) {
            ...
            assert(BigInteger(x / y) == BigInteger(x) / y);
        }
    }
    int cas = 1000000;
    for (int caseNo = 0; cas--; ++caseNo) {
        ...
        if (b) {
            assert(BigInteger(a / b) == BigInteger(a) / b);
        }
    }
}
...
void testInput() {
    ...
    for (int cas = 500000; cas--;) {
        ...
        string src;
        for (int i = 0; i < len; ++i) {
            const string & t = ss[myrandom.nextInt(5)];
...
void testObjectToDouble(cl xxxxx) {
    BigInteger big(xxxxx);
    double dbVal1 = big.doubleValue(), dbVal2;
    stringstream os;
    os << big;
    os >> dbVal2;

    assert(dbVal1 == dbVal1);
...
void testToDouble() {
    for (int cas = 100000; cas--;) {
        LL xxxxx = ((LL) myrandom.nextInt(1 << 20) << 32) + myrandom.nextInt();
        testObjectToDouble(xxxxx);
    }
}
...
void doTheTest() {
    typedef void (*Fun)();
    Fun funs[] = {
        testCompile, test0, test_1, testAdd, testSelfAdd,
        testCompare, testAddOne, testUnaryFunction, testCalc, test_2,
        testShift, test_3, testCalc2, testDivide, testToString10,
        testStringConstructor, testConstructor, testBit, testInput, testToDouble,
        testV1
    };
    time_t st = clock();
